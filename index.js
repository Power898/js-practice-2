...
function calculateExpression(expression) { if(isNaN(expression)) { return eval(expression);} return 'Argument is not a string';
...
function getNumberFromString(s) {let value1=parseFloat(s); if(isNaN(value1)){ return 'Couldnt extract a number from the provided string';}
 return value1;
...
function getIntegerFromString(s) { let number1=parseInt(s); if(isNaN(number1)){return 'Coudent extract an integer from the provided string';}
return number1;
...
function isNumberFinite(num) {if(isFinite(num)) {return 'The number is finite';} return 'The number is not finite';
...
function isValueNaN(value) {if(isNaN(value)) {return 'The value is NaN';} return 'The value is not NaN';
...
function createURLObject(urlStr) { return encodeURI(urlStr);
...
function encodeURLComponent(urlComponent) { return encodeURIComponent(urlComponent);
...
function encodeURL(url) { return encodeURI(url);
...
function decodeURLComponent(urlComponent) {return decodeURIComponent(urlComponent);
...
function decodeURL(url) { return decodeURI(url);
